class_name WorldClock extends Node3D

signal minute_changed(time: Array[int]) # Use to sync world time advancing
...
func load_time(time_array: Array[int]) -> void:
	var load_min := float(time_array[2])
	var load_hour := float(time_array[1])
	var load_day := float(time_array[0])
	
	_time = MINUTES_PER_DAY * MINUTES_PER_HOUR * ((load_min/60) + load_hour + (load_day*24))
...
func _recalculate_time() -> void:
	...
	if _past_minute != minute:
		...
		minute_changed.emit(world_time)
